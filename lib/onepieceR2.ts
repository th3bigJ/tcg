...
export function getOnePieceR2Bucket(): string {
  const bucket = process.env.R2_BUCKET?.trim();
  if (!bucket) throw new Error("R2_BUCKET env var not set");
  return bucket;
}

export function onePieceR2Key(relativePath: string): string {
...
export async function uploadLocalFileToOnePieceR2(
  s3: S3Client,
  absolutePath: string,
  relativePath: string,
): Promise<void> {
  const body = fs.readFileSync(absolutePath);
  await putBufferToOnePieceR2(s3, relativePath, body, guessContentType(absolutePath));
}

export async function putJsonToOnePieceR2(
  s3: S3Client,
  relativePath: string,
  value: unknown,
): Promise<void> {
  await putBufferToOnePieceR2(
    s3,
    relativePath,
...
export async function getJsonFromOnePieceR2<T>(
  s3: S3Client,
  relativePath: string,
): Promise<T | null> {
  try {
