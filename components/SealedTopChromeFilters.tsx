...
function TopChromeChipSelect({
  ...
  return (
    ...
    >
      <span className="whitespace-nowrap opacity-0" aria-hidden="true">{displayLabel}</span>
      ...
      {icon ? <span className={`pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 ${active ? "text-white" : "text-white/78"}`}>{icon}</span> : null}
      {active && clearable ? (
...
function FilterSheetModal({
  ...
  return (
    <div className="fixed inset-0 z-[1004] flex items-end justify-center bg-[var(--foreground)]/45" onClick={onClose} role="presentation">
      <div
      ...
      >
        ...
        <div className="flex items-center justify-between border-b border-[var(--foreground)]/12 px-6 pb-5 pt-4">
          <h2 className="text-xl font-semibold">{title}{selectedCount > 0 ? ` (${selectedCount})` : ""}</h2>
          <button type="button" onClick={onClose} className="inline-flex h-10 w-10 items-center justify-center rounded-full text-[var(--foreground)]/70 transition hover:bg-[var(--foreground)]/10 hover:text-[var(--foreground)]" aria-label={`Close ${title}`}>×</button>
        </div>
        <div className="max-h-[50dvh] overflow-y-auto px-6 py-3">
          <div className="flex flex-col">
            {options.map((option) => {
              ...
              return (
                <button key={option.value || "__empty"} type="button" onClick={() => setDraftValue(option.value)} className="flex items-center gap-4 rounded-xl px-1 py-3 text-left transition hover:bg-[var(--foreground)]/6">
                  <span className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-md border ${selected ? "border-[var(--foreground)] bg-[var(--foreground)] text-[var(--background)]" : "border-[var(--foreground)]/35 text-transparent"}`} aria-hidden="true">
        ...
        <div className="px-6 pb-[max(1.5rem,calc(env(safe-area-inset-bottom,0px)+1rem))] pt-3">
          <button type="button" onClick={() => onApply(draftValue)} className="w-full rounded-full border border-[var(--foreground)]/20 bg-[var(--foreground)] px-5 py-4 text-lg font-semibold text-[var(--background)] transition hover:opacity-90">
            Show results
          </button>
        </div>
      </div>
...
export function SealedTopChromeFilters() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const res = await fetch("/api/sealed/filter-options");
        if (!res.ok) return;
        ...
        if (!cancelled) {
          ...
          setSeriesOptions(json.seriesOptions ?? []);
        }
      } catch {
        // Ignore top chrome filter catalog failures.
      }
    })();
    return () => {
      cancelled = true;
  ...
  const sheetConfig: Record<SheetKey, { title: string; value: string; options: FilterOption[]; onApply: (value: string) => void }> = {
    sort: {
      title: "Sort",
      value: activeSort,
  ...
  return (
    <>
      <div className="pointer-events-auto scrollbar-hide flex items-center gap-2 overflow-x-auto pb-0.5">
        {hasActiveFilters ? <TopChromeClearButton onClick={() => router.push(buildSealedBrowseHref({ search: activeSearch }, { basePath: "/search", tab: "sealed" }))} /> : null}
        <TopChromeChipSelect
          label="Sort"
          active
