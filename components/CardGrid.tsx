...
export function CardGrid({ cards }: { cards: CardEntry[] }) {
  ...
  return (
    <>
      ...
      {selectedCard && (
        <div
          className="fixed inset-0 z-50 flex items-center justify-center bg-[var(--foreground)]/60 p-4"
          onClick={closeModal}
          role="dialog"
          aria-modal="true"
        ...
        >
          <div
            className="relative max-h-[90vh] max-w-[90vw] shrink-0 overflow-hidden rounded-lg border border-[var(--foreground)]/20 bg-[var(--background)] shadow-lg"
          ...
          >
            ...
            <button
              type="button"
              onClick={closeModal}
              className="absolute right-2 top-2 rounded-full bg-[var(--foreground)]/80 p-1.5 text-[var(--background)] transition hover:bg-[var(--foreground)]"
              aria-label="Close"
            >
