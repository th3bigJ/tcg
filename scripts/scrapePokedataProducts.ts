...
const mode = readArgValue("--mode=") || "all";
const requestedTcg = readArgValue("--tcg=") || DEFAULT_TCG;
const requestedLanguageRaw = readArgValue("--language=");
const requestedLanguage = normalizeFilterValue(requestedLanguageRaw) ?? DEFAULT_LANGUAGE;
const imageConcurrency = parsePositiveInt(readArgValue("--image-concurrency=")) ?? DEFAULT_IMAGE_CONCURRENCY;
...
function buildSlugParts(): string[] {
...
function buildCatalogPayload(products: PokedataProduct[]): ProductCatalogPayload {
  return {
...
function buildPricesPayload(products: PokedataProduct[]): PricePayload {
  return {
    scrapedAt: new Date().toISOString(),
    sourceUrl: SOURCE_URL,
    sourceApiUrl: SOURCE_API_URL,
...
async function uploadProductImages(
  ...
  await mapPool(products, imageConcurrency, async (product, index) => {
    try {
      const result = await uploadImageForProduct(s3, bucket, product);
  ...
  if (failures.length > 0) {
    const failuresPath = path.join(OUTPUT_DIR, `${buildSlugParts().join("-")}-image-failures.json`);
    ...
    const reportKey = `sealed-products/pokedata/${buildSlugParts().join("-")}-image-failures.json`;
...
async function main(): Promise<void> {
  if (!["all", "products", "prices"].includes(mode)) {
    throw new Error(`Unsupported mode "${mode}". Use --mode=all, --mode=products, or --mode=prices.`);
  }
  ...
  const slug = buildSlugParts().join("-");

  const catalogPayload = buildCatalogPayload(filteredProducts);
  const pricesPayload = buildPricesPayload(filteredProducts);
  ...
  if (mode === "all" || mode === "products") {
    const imageUploadResult = await uploadProductImages(s3, bucket, filteredProducts);
